...
/*
    if (numOfPlayers == 2){
        io.in('game-room').emit('start_game');
        console.log("starting game");
    }
    else
        console.log("Helvete 1");
*/

    socket.on('key_pressed', function(data){
        //io.in('game-room').emit('key_pressed', {playerId: socket.id, key: data.key ,state:'down'});
        //console.log("key press received: ", data.key);
    })

    socket.on('key_up', function(data){
        //io.in('game-room').emit('key_pressed', {playerId: socket.id, key: data.key, state:'up'});
...
/*
    socket.on('move_player', function (data) {
        ...
        console.log('move player received, data:');
        console.log(data);
        if (data.info == 0)
            players[socket.id].this.body.rotateLeft(100);
            //players[socket.id].rotateLeft(100);
